...
# Create router
router = APIRouter(
    prefix="/api/content-test",
    tags=["content-test"],
    responses={404: {"description": "Not found"}},
)

# Models
class ContentTestQuestion(BaseModel):
    id: int
...
class ContentTestResponse(BaseModel):
    status: str
    message: str
    questions: List[ContentTestQuestion]
    topics: List[str]
    file_name: str

@router.post("/analyze", response_model=ContentTestResponse)
async def analyze_file_and_generate_test(
    request: Request,
    file: UploadFile = File(...),
    exam_type: Optional[str] = Form(None)
):
    """
    Analyze the uploaded file and generate a custom mock test based on its content
    """
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info(f"Extracted {len(truncated_text)} characters of text")
            ...
            return ContentTestResponse(
                status="success",
                message="Content analyzed and mock test generated successfully",
                questions=questions,
                topics=topics,
        ...
        logger.error(f"Error analyzing file and generating test: {str(e)}")
...
async def generate_questions(text: str, topics: List[str], exam_type: Optional[str] = None) -> List[ContentTestQuestion]:
    """
    ...
    try:
        ...
        prompt = f"""
        Generate a comprehensive mock test based on the following text content{context}.
        ...
        Create 10 questions that specifically test knowledge from this content. Include a mix of:
        ...
        IMPORTANT: All questions MUST be directly based on the specific content provided. Do NOT create generic questions.
        Each question should test understanding of specific information, concepts, or facts from the text.
        """
        ...
        if isinstance(result, list) and len(result) > 0:
            ...
            for i, q in enumerate(result):
                ...
                questions.append(ContentTestQuestion(**question))
...
async def generate_analysis(correct_answers: int, total_questions: int, daily_learning_capacity: int) -> str:
    ...
    try:
        ...
        prompt = f"""
        Generate a personalized analysis for a student who has completed a mock test based on their study material.
